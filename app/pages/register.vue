    ...
    <UCard class="w-full max-w-md bg-white/5 dark:bg-black/20 backdrop-blur-xl border-default shadow-2xl">
      <template #header>
        <div class="text-center">
          ...
          <p class="text-muted-foreground text-sm mt-1">加入秘密基地，开启你的旅程</p>
        </div>
      </template>

      <form @submit.prevent="handleRegister" class="space-y-4">
        <UFormField label="用户名">
          <UInput v-model="form.username" placeholder="如何称呼你？" icon="i-heroicons-user" class="w-full" />
        </UFormField>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <UFormField label="确认密码">
            <UInput v-model="form.confirmPassword" type="password" placeholder="••••••••"
              icon="i-heroicons-shield-check" class="w-full" />
          </UFormField>
        </div>

        <UFormField label="邀请码 (可选)">
          <UInput v-model="form.invitationCode" placeholder="如果你有邀请码..." icon="i-heroicons-ticket" class="w-full" />
        </UFormField>
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (!response.error) {
      ...
      toast.add({ title: "Registration failed", description: (response.error as any).message, color: "error" });
    }
  } finally {
    loading.value = false;
  }
