<template>
  <div class="flex flex-col h-[600px] bg-slate-950 rounded-xl border border-slate-800 font-mono shadow-2xl overflow-hidden">
    ...
    <div ref="outputRef" class="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
      ...
      <div v-for="(log, index) in logs" :key="index" :class="getLogClass(log.type)" class="text-sm border-l-2 pl-3 bg-slate-900/30 py-1">
        <div class="flex justify-between opacity-50 text-[10px] mb-1">
          <span>{{ log.type.toUpperCase() }}</span>
          <span>{{ log.time }}</span>
        </div>
        <pre class="whitespace-pre-wrap break-all leading-relaxed">{{ log.content }}</pre>
      </div>
    </div>
    ...
    <div class="p-4 bg-slate-900/80 border-t border-slate-800 backdrop-blur-md">
      <div class="flex gap-2 items-center">
        ...
        <div class="flex gap-1">
          <UButton color="neutral" variant="ghost" icon="i-heroicons-trash" @click="logs = []" />
          <UButton color="primary" variant="subtle" @click="executeCode">RUN</UButton>
        </div>
      </div>
    </div>
  </div>
...
const executeCode = async () => {
  ...
  try {
    ...
    const fn = new Function(...keys, `return (async () => { return ${code} })()`)
    const result = await fn(...values)

    addLog('out', typeof result === 'object' ? JSON.stringify(result, null, 2) : String(result))
  } catch (err: any) {
