...
const i18nMap: Record<string, { label: string; description?: string; icon?: string }> = {
  ...
  'site.seo.general.title': { label: '站点标题', description: '浏览器标签页显示的文字' },
  'site.seo.general.description': { label: '站点描述', description: '用于搜索引擎检索的简短介绍' },
  'site.seo.general.keywords': { label: '关键词', description: '多个关键词请使用英文逗号分隔' },
  'site.seo.general.robots': { label: 'Robots 协议', description: '控制搜索引擎爬虫的抓取行为 (index, follow)' },
  'site.seo.og': { label: '社交分享 (OpenGraph)' },
  'site.seo.og.title': { label: '分享标题', description: '社交媒体分享时显示的标题' },
  'site.seo.og.description': { label: '分享描述', description: '社交媒体分享时显示的详情文字' },
  'site.seo.og.image': { label: '分享预览图', description: '社交媒体分享时显示的图片 URL' },
  'site.seo.social': { label: '社交媒体卡片' },
  'site.seo.social.twitter_card': { label: 'Twitter 卡片类型', description: '定义推文分享时的展示布局' },
  'site.security.cookie': { label: 'Cookie 安全' },
  'site.security.cookie.auto_renew': { label: '自动续期', description: '用户活跃时自动延长登录状态' },
  'site.security.cookie.expire_hours': { label: '有效期 (小时)', description: '登录状态保持的时长' },
  'site.user.registration': { label: '注册设置' },
  'site.user.registration.enabled': { label: '开放注册', description: '是否允许新用户注册账号' },
  'site.user.registration.force_invitation': { label: '强制邀请码', description: '注册时是否必须填写邀请码' },
  'site.home.background': { label: '背景展示' },
  'site.home.background.url': { label: '图片地址', description: '背景图的远程或本地链接' },
  'site.home.background.blur': { label: '模糊度 (px)', description: '背景图的高斯模糊程度' },
  'site.home.background.opacity': { label: '不透明度', description: '范围 0.0 到 1.0' },
  'site.home.banner': { label: '横幅通告' },
  'site.home.banner.content': { label: '内容文本', description: '首页醒目位置显示的文字' },
  'site.home.banner.display_mode': { label: '显示模式', description: '控制横幅的视觉覆盖范围 (full/screen)' },
  'site.footer.beian': { label: '备案与合规' },
  'site.footer.beian.icp': { label: 'ICP 备案号', description: '例如：京ICP备XXXXXXXX号' },
  'site.footer.beian.police': { label: '公安备案号', description: '网站底部的公安联网备案信息' }
}
...
const fetchData = async () => {
  ...
  try {
    ...
    if (response.error || !response.data) throw new Error("Unable to fetch site settings.");
...
const isModified = (item: SettingListItem) => editableSettings.value[item.key] !== item.currentValue
...
<template>
  <UContainer class="py-8">
    <div class="mb-6 flex items-center justify-between">
      ...
      <UBadge v-if="loading" color="secondary" variant="soft">Loading...</UBadge>
    </div>
    ...
    <UTabs :items="tabs" class="w-full">
      <template v-for="tabItem in tabs" #[tabItem.slot]="{ item }">
        <div class="space-y-6 mt-4">
          <UCard
            v-for="(items, cardKey) in groupedSettings[item.slot.replace(/_/g, '.')]"
            :key="cardKey"
          >
            <template #header>
              <h3 class="font-semibold uppercase tracking-wider text-sm text-gray-600 dark:text-gray-400">
              ...
              >
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-sm font-medium">{{ getSubKeyLabel(setting.key) }}</span>
                    <UTooltip v-if="i18nMap[setting.key]?.description" :text="i18nMap[setting.key]?.description">
                      <UIcon name="i-lucide-info" class="size-4 text-gray-400 cursor-help" />
                    </UTooltip>
                  </div>
                  <div class="text-xs text-gray-400 font-mono mt-0.5">{{ setting.key }}</div>
                </div>
                ...
                <div class="flex items-center gap-3 shrink-0">
                  ...
                  <template v-else-if="setting.type === 'int' || setting.type === 'double'">
                    <UInput
                  ...
                  <template v-else>
                    <UInput v-model="editableSettings[setting.key]" class="min-w-[280px]" />
                  </template>

                  <div class="flex items-center gap-1 min-w-[100px] justify-end">
                    <UButton
