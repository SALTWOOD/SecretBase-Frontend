...
// 直接绑定当前选中的代码字段
const code = computed({
  get: () => currentShortcode.value ? (currentShortcode.value[editMode.value] ?? '') : '',
  set: (val) => {
...
// 4. 操作方法
const selectItem = (id: string | number, mode: 'backendCode' | 'frontendCode') => {
  currentId.value = id
  editMode.value = mode
}
...
<template>
  <div class="flex h-screen flex-col overflow-hidden bg-[#282c34] text-gray-200 font-sans text-sm">
    <header class="flex h-12 items-center justify-between border-b border-gray-700 px-4 shrink-0 bg-[#21252b]">
      <div class="flex items-center gap-2">
        <UIcon name="i-lucide-code-2" class="h-5 w-5 text-primary" />
        <span class="font-bold tracking-tight">简码编辑器</span>
        <div v-if="currentShortcode" class="flex items-center gap-1 ml-4 opacity-60">
          <UIcon name="i-lucide-chevron-right" class="w-3 h-3" />
          <span class="text-xs font-mono">{{ currentShortcode.name }}</span>
          <UIcon name="i-lucide-chevron-right" class="w-3 h-3" />
          <span class="text-xs text-primary">{{ editMode === 'backendCode' ? 'backend.js' : 'frontend.js' }}</span>
        </div>
      </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      <aside class="w-72 border-r border-gray-700 bg-[#21252b] flex flex-col shrink-0">
        <div class="p-4 flex-1 overflow-y-auto custom-scrollbar">
          <h3 class="text-[10px] font-bold uppercase text-gray-500 mb-4 tracking-[0.2em]">Project Explorer</h3>

          <nav class="space-y-4">
            <template v-if="isLoading">
              <USkeleton v-for="i in 4" :key="i" class="h-16 w-full bg-gray-800/40" />
            </template>

            <div v-for="item in shortcodes" :key="item.id" class="space-y-1">
              <div class="px-2 py-1 flex items-center gap-2 text-gray-400 group cursor-default">
                <UIcon name="i-lucide-folder-code" class="w-4 h-4 opacity-50" />
                <span class="text-[11px] font-semibold truncate">{{ item.displayName || item.name }}</span>
              </div>
    ...
    <footer class="h-8 border-t border-gray-700 px-4 flex items-center justify-between text-[10px] text-gray-500 bg-[#1c1f24] shrink-0 font-mono">
...
:deep(.cm-scroller) { font-family: 'JetBrains Mono', 'Fira Code', monospace; }
