...
const fileName = computed(
  () => fileKey.value.split("/").pop() || fileKey.value,
);
...
const updateByte = (index: number, event: Event) => {
  ...
  el.innerText = rawBytes.value[index]
    ?.toString(16)
    .padStart(2, "0")
    .toUpperCase()!;
};
...
const toAscii = (bytes: any[]) => {
  return bytes
    .map((b) =>
      b.value >= 32 && b.value <= 126 ? String.fromCharCode(b.value) : ".",
    )
    .join("");
};
  ...
  <UContainer class="py-6">
    <header class="mb-6">
      <div class="flex justify-between items-end mb-4">
        ...
        <div class="flex items-center gap-3">
          <div
            class="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-800 p-1 rounded-md border border-zinc-200 dark:border-zinc-700"
          >
            <UButton
              :variant="viewMode === 'text' ? 'solid' : 'ghost'"
              size="xs"
              color="neutral"
              icon="i-lucide-type"
              @click="viewMode = 'text'"
            ...
            />
            <UButton
              :variant="viewMode === 'hex' ? 'solid' : 'ghost'"
              size="xs"
              color="neutral"
    ...
    >
      <UIcon
        ...
        class="animate-spin size-8 text-primary/50"
      />
    </div>
    ...
    <div v-else class="space-y-4">
      <UAlert
        ...
        :description="
          editLock === 'hex'
            ? 'Changes made in HEX mode. Text editing is disabled.'
            : 'Changes made in Text mode. HEX editing is disabled.'
        "
      />

      <div
        class="relative border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-zinc-950"
      >
        <textarea
          v-if="viewMode === 'text'"
          v-model="content"
        ...
        <div
          ...
          class="w-full h-[70vh] p-4 overflow-auto prose dark:prose-invert max-w-none"
        >
          <MarkdownEditor
            v-model="content"
            :disabled="editLock === 'hex'"
            @input="onTextUpdate"
          />
        </div>
            ...
            >
              <tbody>
                <tr
                  ...
                  class="border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-primary-500/5 transition-colors"
                >
                  <td
                    class="px-4 text-zinc-400 select-none w-24 text-right pr-6"
                  >
                    {{ row.address }}
                  </td>
                    ...
                    <span
                      ...
                      :class="
                        editLock === 'text'
                          ...
                          : 'hover:bg-primary-500/20 cursor-text'
                      "
                  ...
                  <td
                    class="px-6 text-zinc-500 border-l border-zinc-100 dark:border-zinc-800 whitespace-pre"
                  >
                    {{ toAscii(row.bytes) }}
                  </td>
                </tr>
        ...
        <div
          class="px-3 py-1.5 flex justify-between items-center text-[10px] text-zinc-400 bg-zinc-50/50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800"
        >
          <div class="flex gap-4">
            <span class="flex items-center gap-1"
              ><UIcon name="i-lucide-hard-drive" />
              {{ formatSize(Number(fileMetadata?.contentLength || 0)) }}</span
            >
            <span class="uppercase">Mode: {{ viewMode }}</span>
          </div>
          <div class="flex items-center gap-3">
            <span v-if="hasChanges" class="text-amber-500 font-bold italic"
