...
const toggleStatus = async (row: FileShareResponse) => {
  ...
  try {
    ...
    toast.add({ title: 'Update Failed', description: 'Server rejected the change', color: 'error' });
  }
}
  ...
  <UContainer class="py-6">
    <UCard>
      ...
      <div class="flex gap-4 mb-4">
        <UInput placeholder="搜索分享..." icon="i-heroicons-magnifying-glass" class="w-64" />
        <USelectMenu placeholder="按 Bucket 查找" :options="['my-bucket']" class="w-48" />
      </div>
      ...
      >
        <template #short-id-cell="{ row }">
          <span class="font-mono text-xs text-gray-500">{{ row.original.shortId }}</span>
        </template>
        ...
        <template #is-enabled-cell="{ row }">
          <USwitch v-model="row.original.isEnabled" @update:model-value="toggleStatus(row.original)" />
        </template>
        ...
        <template #actions-cell="{ row }">
          <div class="flex gap-2">
            ...
            <ConfirmButton
              ...
              color="error"
              title="确认删除吗？"
              @confirm="(confirmed) => confirmed && deleteShare(row.original.shortId)"
            />
          </div>
        </template>
      </UTable>
